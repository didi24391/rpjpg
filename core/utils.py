...
def create_video(video_name, fps, output_dir):
    ...
    os.system(f'ffmpeg -framerate {fps} -i "{output_dir}{sep}%04d.jpg" -c:v libx264 -crf 23 -pix_fmt yuv420p -y "{output_dir}{sep}output.mp4"')


def extract_frames(input_path, output_dir):
    input_path, output_dir = path(input_path), path(output_dir)
    os.system(f'ffmpeg -i "{input_path}" "{output_dir}{sep}%04d.jpg"')
